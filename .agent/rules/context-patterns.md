...
export const FinancialProvider: React.FC<ProviderProps> = ({ children }) => {
  ...
  const editEntity = useCallback(async (id: string, data: Partial<FormData>) => {
...
const MyComponent: React.FC = () => {
  const { entities, isLoading, createEntity, removeEntity } = useFinancial();
  ...
  return (
    <div>
      {entities.map((entity) => (
        <EntityCard key={entity.id} entity={entity} onDelete={() => removeEntity(entity.id)} />
      ))}
    </div>
  );
};
